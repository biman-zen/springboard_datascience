...
the member name. */
SELECT   
	DISTINCT CONCAT_WS(" ",m.firstname,m.surname) AS member,
	f.name AS court
FROM Bookings AS b
...
SELECT 
	...
	SUM(CASE WHEN b.memid != 0 THEN f.membercost*b.slots ELSE 0 END) AS revenue
FROM Bookings AS b
...
SELECT 
	...
	CASE WHEN b.memid != 0 THEN b.slots ELSE 0 END AS number_slots
...
/*
SELECT
	MONTHNAME(b.starttime) AS month,
	f.name AS facility,
